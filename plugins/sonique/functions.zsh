...
function startVdcDatabase {
    ...
    if [[ $? -eq 1 ]]; then
        ...
        docker run --name oracle-12c-vdc -d -p 1525:1521 -p 5505:5500 --shm-size=2g --restart=unless-stopped --net=sonique-network --net-alias=oracle-12c-vdc repo.sns.sky.com:8085/sns-is-dev/oracle-12c-vdc:11 > /dev/null
    else
        if [[ $output == 'running' ]]; then
            #do nothing
        else
...
function runBattenbergLoaderJob {
    ...
    (set -x; docker run --rm --name battenberg-loader --net=sonique-network --net-alias=battenberg-loader -v /data:/app/data  -e "cluster.host=sonique-cluster.sns.sky.com" -e "npr.volume.mount.path=/app/npr" -e "replicas=3" -e "npr.volume.host.server=vm002544.bskyb.com" -e "repo.host=repo.sns.sky.com" -e "npr.volume.host.path=/home/sonique/nfs/npr" -e "npr.volume.name=npr-ftp" -e "limits.memory=4Gi" -e "nodePort=30030" -e "repo.port=8085" -e "jdbc.transaction.context.factory.class=sonique.sql.transaction.factory.OracleTransactionContextFactory" -e "service.summary.status.path=status" -e "app.data.integrity.ignore.window.mins=15" -e "app.data.retention.period.in.days=30" -e "loader.schedule=*/1 * * * *" -e "jdbc.connection.user=battenberg_user" -e "app.file.directory=/app/data/npr" -e "jdbc.connection.password=battenberg" -e "jdbc.connection.url=jdbc:oracle:thin:@//oracle-12c-vdc:1521/db1" -e "app.port=8087" -e "jdbc.connection.driver=oracle.jdbc.pool.OracleDataSource" -e "database.edition=BATTENBERG_2" -e "service.summary.lookup.path=service" -e "service.summary.base.uri=http://$ip_addr:11565/repoman/" -e TZ=Europe/London repo.sns.sky.com:8085/sns-is-dev/battenberg-loader:$battenberg_loader_version)
}
...
function runBullwinkleNotifierJob {
    ...
    local ip_addr=$(ipconfig getifaddr en1)
    if [ -e $ip_addr ]; then
        $ip_addr = $(ipconfig getifaddr en0)
    fi
    (set -x; docker run --rm --name bullwinkle-notifier '--net=sonique-network' '--net-alias=bullwinkle-notifier' -v /data:/app/data -e 'jdbc.transaction.context.factory.class=sonique.sql.transaction.factory.OracleTransactionContextFactory' -e 'jdbc.connection.user=bullwinkle_user' -e 'jdbc.connection.url=jdbc:oracle:thin:@//oracle-12c:1521/db1' -e 'jdbc.connection.password=bullwinkle' -e 'jdbc.connection.driver=oracle.jdbc.pool.OracleDataSource' -e 'database.edition=BULLWINKLE_6' -e TZ=Europe/London -e 'operator.port=11565' -e "operator.hostAddress=http://$ip_addr" -e 'operator.writeEndpoint=/troll/llustreamplus/web/showAndTellController.html' -e 'operator.statusEndpoint=/troll/status' -e "app.maxEntries=30000" -e "app.port=8080" -e "app.notificationDeliveryAttemptLimit=5" -e "app.requestTimeoutInMinutes=5" repo.sns.sky.com:8085/sns-is-dev/bullwinkle-notifier:$bullwinkle_notifier_version)
}
