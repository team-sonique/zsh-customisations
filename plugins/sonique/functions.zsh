...
function startVdcDatabase {
    ...
    if [[ $? -eq 1 ]]; then
        ...
        docker run --name oracle-12c-vdc -d -p 1525:1521 -p 5505:5500 --shm-size=2g --restart=unless-stopped --net=sonique-network --net-alias=oracle-12c-vdc repo.sns.sky.com:8085/dost/oracle-12c-vdc:74 > /dev/null
    else
        if [[ $output == 'running' ]]; then
            #do nothing
        else
