...
function runBullwinkleWriterJob {
    ...
    (set -x; docker run --rm --name bullwinkle-file-writer --net=sonique-network --net-alias=bullwinkle-file-writer -v /data:/app/data -e "jdbc.transaction.context.factory.class=sonique.sql.transaction.factory.OracleTransactionContextFactory" -e "app.maxEntries=30000" -e "app.port=8080" -e "app.notificationDeliveryAttemptLimit=5" -e "app.requestTimeoutInMinutes=5" -e "jdbc.connection.user=bullwinkle_user" -e "jdbc.connection.url=jdbc:oracle:thin:@//oracle-12c-vdc:1521/db1" -e "jdbc.connection.password=bullwinkle" -e "jdbc.connection.driver=oracle.jdbc.pool.OracleDataSource" -e "database.edition=BULLWINKLE_1" -e TZ=Europe/London -e "tuk.record.actionUnblock=R" -e "tuk.record.operator=SKY" -e "tuk.record.blockReasonCode=0011" -e "tuk.record.filePrefix=SKY_CEIR_" -e "tuk.record.unblockSource=Removed on behalf of Sky" -e "tuk.record.headerIdentifier=10" -e "tuk.record.recordSpecificationVersion=01" -e "tuk.record.createdDatePattern=yyMMdd" -e "tuk.record.colouredList=B" -e "tuk.record.unblockReasonCode=0014" -e "tuk.record.actionBlock=I" -e "tuk.record.blockSource=Blocked on behalf of Sky" -e "tuk.record.organisationId=GBRKY" -e "tuk.record.bodyIdentifier=55" -e "tuk.record.destDir=/app/data/tuk/outgoing" -e "tuk.record.footerIdentifier=90" -e "tuk.record.fileExtension=UPD" -e "tuk.record.writeDir=/app/data/tuk/outgoing" repo.sns.sky.com:8085/sns-is-dev/bullwinkle-file-writer:$bullwinkle_writer_version)
}
...
function runBullwinkleReaderJob {
    ...
    (set -x; docker run --rm --name bullwinkle-file-reader --net=sonique-network --net-alias=bullwinkle-file-reader -v /data:/app/data -e "jdbc.transaction.context.factory.class=sonique.sql.transaction.factory.OracleTransactionContextFactory" -e "jdbc.connection.user=bullwinkle_user" -e "jdbc.connection.url=jdbc:oracle:thin:@//oracle-12c-vdc:1521/db1" -e "jdbc.connection.password=bullwinkle" -e "jdbc.connection.driver=oracle.jdbc.pool.OracleDataSource"  -e "database.edition=BULLWINKLE_6" -e TZ=Europe/London -e "tuk.record.filePrefix=SKY_CEIR_" -e "file.retentionPeriodInDays=7" -e "tuk.record.destinationDir=/app/data/tuk/incoming" -e "tuk.record.readDir=/app/data/tuk/incoming" repo.sns.sky.com:8085/sns-is-dev/bullwinkle-file-reader:$bullwinkle_reader_version)
}
